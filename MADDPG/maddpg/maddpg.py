...
class MADDPG:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        path = ('./model/simple_spread'+ '/' + 'agent_%d' % agent_id)
        # 加载模型
        if os.path.exists(path + '/actor_params.pkl') and self.args.evaluate == True:
        ...
        # 加载模型
        if os.path.exists(path + '/actor_params.pkl') and self.args.load_model == True and self.args.evaluate == False:
            self.actor_network.load_state_dict(torch.load(path + '/actor_params.pkl'))
            self.critic_network.load_state_dict(torch.load(path + '/critic_params.pkl'))
            print('Agent {} successfully loaded actor_network: {}'.format(self.agent_id,
                                                                          path + '/actor_params.pkl'))
